...
def _vote_msg(message_storage: storage.MessageVoteStorage) -> str:
    vote_count = len(message_storage.votes)
    if message_storage.is_revealed:
        ...
        vote_values = ' '.join(v.value for v in message_storage.votes.values())

        vote_maxwidth = max(
            _get_string_length_px(_make_vote_line(author, vote))
            for author, vote in message_storage.votes.items()
        )
        answers_per_user = []
        for author, vote in message_storage.votes.items():
        ...
        reveal_status = ''
        voter_names = ', '.join(voter.mention for voter in message_storage.votes.keys())
...
async def vote(message_storage: storage.MessageVoteStorage, user: discord.User, value: str) -> None:
    message_storage.votes[user] = storage.Vote(author=user, value=value)
